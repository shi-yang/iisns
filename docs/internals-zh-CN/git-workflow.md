Git 的工作流程 - 贡献 iiSNS 代码
================================

你想参与到 iiSNS 的开发？很好！但为了能让你快速上手，请遵循以下步骤。
如果你是一个 Git 新手，对 GitHub 不熟悉，你应首先要查看 [GitHub 帮助](http://help.github.com/), [使用 Git](https://try.github.com) 或者学习其它关于 [Git internal data model](http://nfarina.com/post/9868516270/git-is-simpler)。

准备你的开发环境
----------------

以下步骤将为 iiSNS 创建一个开发环境，这将让你使用 iiSNS 来工作，在你贡献代码之前应先完成这些步骤。

### 1. [Fork](http://help.github.com/fork-a-repo/) the iiSNS repository on GitHub and clone your fork to your development environment

```
git clone git@github.com:YOUR-GITHUB-USERNAME/iisns.git
```
...
### 2. Add the main iiSNS repository as an additional git remote called "upstream"

Change to the directory where you cloned iiSNS, normally, "iisns". Then enter the following command:

```
git remote add upstream git://github.com/shi-yang/iisns.git
```
...
- run `php build/build dev/app basic` to clone the basic app and install composer dependencies for the basic app.
  This command will install foreign composer packages as normal but will link the iisns repo to
...
**Now you have a working playground for hacking on iiSNS**
...
normally do e.g. add `"yiisoft/iisns-redis": "*"` to the `require` section of the basic app.
Running `php build/build dev/app basic` will install the extension and its dependecies and create
a symlink to `extensions/redis` so you are not working in the composer vendor dir but in the iisns repository directly.
...
### 2. Fetch the latest code from the main iiSNS branch
...
### 3. Create a new branch for your feature based on the current iiSNS master branch
...
编辑名为 “CHANGELOG.md” 的文件来写下你的改变。你应该在文件的顶部，在标题的下面，开启一行来记录你所做的改变。
这一行应是下面这样的：

```
Bug #999: 修复的 Bug 的描述 (你的姓名)
Enh #999: a description of the enhancement (你的姓名)
```
...
### 7. Pull the latest iiSNS code from upstream into your branch
...
you should fix them now and commit the changes again. This ensures that it's easy for the iiSNS team to merge your changes
with one click.
...
and become part of the next iiSNS release. If not, don't be disheartened, different people need different features and iiSNS
...
To detect regressions early every merge to the iiSNS codebase on GitHub will be picked up by
...
```
git clone git@github.com:YOUR-GITHUB-USERNAME/iisns.git
git remote add upstream git://github.com/yiisoft/iisns.git
```
